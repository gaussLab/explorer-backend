...
public class SyncFecJob {
    ...
    private String chainName = "fec";

    private Long startNum;

    @Scheduled(fixedDelay = 1000)
    public void syncBlock() {
        log.info("start SyncFecJob");
        GlobalBlock globalBlock = blockService.getGlobal(chainName);
        long newBlockNum = CosmosUtil.getNewBlock(configService.getConfig(ConfigUtil.fecUrl));
        ...
        for (long i = startNum; i < newBlockNum; i++) {
            JSONObject b = CosmosUtil.getBlockByHeight(configService.getConfig(ConfigUtil.fecUrl),i);
    ...
    public void saveTransaction(JSONArray txs){
        ...
        transactions.stream().forEach(txJson ->{
            ...
            JSONObject transfer = CosmosUtil.getTxByhash(configService.getConfig(ConfigUtil.fecUrl)+"/gauss",hash);
            if (null != transfer){
                JSONObject tx = TransferService.getTransfer(transfer,chainName,configService.getConfig(ConfigUtil.fecUrl));
