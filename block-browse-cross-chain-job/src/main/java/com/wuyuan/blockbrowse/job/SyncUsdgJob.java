...
public class SyncUsdgJob {
    ...
    private String chainName = "usdg";

    private Long startNum;

    @Scheduled(fixedDelay = 1000)
    public void syncBlock() {
        log.info("start SyncUsdgJob");
        GlobalBlock globalBlock = blockService.getGlobal(chainName);
        long newBlockNum = CosmosUtil.getNewBlock(configService.getConfig(ConfigUtil.usdgUrl));
        ...
        for (long i = startNum; i < newBlockNum; i++) {
            JSONObject b = CosmosUtil.getBlockByHeight(configService.getConfig(ConfigUtil.usdgUrl),i);
    ...
    public void saveTransaction(JSONArray txs){
        ...
        transactions.stream().forEach(txJson ->{
            ...
            JSONObject transfer = CosmosUtil.getTxByhash(configService.getConfig(ConfigUtil.usdgUrl)+"/usdg",hash);
            if (null != transfer){
                JSONObject tx = TransferService.getTransfer(transfer,chainName,configService.getConfig(ConfigUtil.usdgUrl));
