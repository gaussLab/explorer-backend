...
public class SyncGpbJob {
    ...
    private String chainName = "gpb";

    private Long startNum;

    @Scheduled(fixedDelay = 1000)
    public void syncBlock() {
        log.info("start SyncGpbJob");
        GlobalBlock globalBlock = blockService.getGlobal(chainName);
        long newBlockNum = CosmosUtil.getNewBlock(configService.getConfig(ConfigUtil.gpbUrl));
        ...
        for (long i = startNum; i < newBlockNum; i++) {
            JSONObject b = CosmosUtil.getBlockByHeight(configService.getConfig(ConfigUtil.gpbUrl),i);
    ...
    public void saveTransaction(JSONArray txs){
        ...
        transactions.stream().forEach(txJson ->{
            ...
            JSONObject transfer = CosmosUtil.getTxByhash(configService.getConfig(ConfigUtil.gpbUrl)+"/gauss",hash);
            if (null != transfer){
                JSONObject tx = TransferService.getTransfer(transfer,chainName,configService.getConfig(ConfigUtil.gpbUrl));
